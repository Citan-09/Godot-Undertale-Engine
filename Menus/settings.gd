...
@onready var soul: Sprite2D = $MarginContainer/Soul
@onready var defaultsize: Vector2 = $MarginContainer.size
@onready var defaultpos: Vector2 = position
@export var time: float = 1.0

func parse_setting_to_text(setting_value: Variant) -> Variant:
	...
	if enabled:
		...
		if event.is_action_pressed("ui_up"):
			if souly > 0:
				$choice.play()
				souly -= 1
		var currentsetting: String = Global.settings.keys()[souly]
